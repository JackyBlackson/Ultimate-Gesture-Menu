package com.jackyblackson.modfabric.ultimate.gesture.menu.bstats;
...
public class BstatsMetrics {
    ...
    private final FabricModRepr thisFabricMod;
    ...
    public static class FabricModRepr {
        private final String modId;

        private final ModContainer container;

        public FabricModRepr(String modId) {
            this.modId = modId;
            container = FabricLoader.getInstance().getModContainer(modId).orElseThrow(() -> new RuntimeException("Mod not found!"));
        }
        ...
        public boolean getOnlineMod() {
            return true;
        }

        public Logger getLogger() {
            return LoggerFactory.getLogger(this.modId);
        }
    }
     ...
     */
    public BstatsMetrics(String modId, int serviceId, boolean logErrors, boolean logSentData, boolean logResponseStatusText) {

        this.thisFabricMod = new FabricModRepr(modId);
        this.logger = thisFabricMod.getLogger();
        ...
        metricsBase = new MetricsBase(
                ...
                submitDataTask -> ClientLifecycleEvents.CLIENT_STARTED.register(server -> {
                    logger.warn("bStats is now sending data to its server!!!");
                    server.execute(submitDataTask);
                }),
                () -> true,
                logger::warn,
                logger::info,
    ...
    private void appendPlatformData(JsonObjectBuilder builder) {
        ...
        builder.appendField("bukkitVersion", thisFabricMod.getFabricVersion());
        builder.appendField("bukkitName", "bukkit");
    ...
    private void appendServiceData(JsonObjectBuilder builder) {
        builder.appendField("pluginVersion", thisFabricMod.getModVersion());
    }
    ...
    public static class MetricsBase {
        ...
        public MetricsBase(
            ...
            if (enabled) {
                ...
                // bStats
                infoLogger.accept("bStats is trying to start submit data");
                startSubmitting();
                infoLogger.accept("bStats finishes to start submit data");
            } else {
                infoLogger.accept("bStats is disabled");
            }
        }
